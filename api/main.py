...
async def lifespan(app: FastAPI):
    # run the blocking model loader in a thread to avoid blocking the event loop
    await asyncio.to_thread(load_model)
    try:
        yield
    finally:
        # optional: add model cleanup here
        pass
...
def load_model():
    """Load YOLOv8 model on API startup (synchronous).

    This keeps the original loader body but runs under the lifespan
    manager via `asyncio.to_thread` so it doesn't block the event loop.
    """
    global model
